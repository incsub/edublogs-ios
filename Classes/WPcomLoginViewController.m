...
- (void)viewDidLoad {
	...
	WPcomXMLRPCUrl = @"http://edublogs.org/xmlrpc.php";
	...
	if(indexPath.section == 1) {
		...
		UITableViewTextFieldCell *loginCell = [[[UITableViewTextFieldCell alloc] initWithStyle:UITableViewCellStyleDefault 
				...
				if ([indexPath row] == 0) {
					...
					loginCell.textField.placeholder = NSLocalizedString(@"Edublogs username", @"");
					...
					loginCell.textField.placeholder = NSLocalizedString(@"Edublogs password", @"");
...
- (void)authenticateWithSuccess:(void (^)())success failure:(void (^)(NSError *error))failure {
    ...
    [api authenticateWithSuccess:^{
        isAuthenticated = YES;
        [self saveLoginData];
		// Register this device for push notifications with Edublogs if necessary
