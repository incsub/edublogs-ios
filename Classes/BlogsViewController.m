		...
		if(([[prefs objectForKey:@"launch_count"] isEqualToNumber:[NSNumber numberWithInt:80]]) && 
		   ([prefs objectForKey:@"has_displayed_rating_prompt"] == nil)) {
			...
			UIAlertView *ratingAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"App Store Rating", @"") 
																  message:NSLocalizedString(@"If you like Edublogs for iOS, we'd appreciate it if you could leave us a rating in the App Store. Would you like to do that now?", @"") 
																 delegate:self 
...
- (void)setupPhotoButton {
    ...
    if (wantsPhotoButton && quickPhotoButton == nil) {
        ...
        CGFloat x =  0;
        CGFloat width = wantsReaderButton ? (self.view.bounds.size.width) : self.view.bounds.size.width;
...
- (void)showReader { 
    ...
    if ( appDelegate.wpcomAvailable == NO ) {
        UIAlertView *connectionFailAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Sorry, no connection to Edublogs.", @"")
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
	if(buttonIndex == 1) {
		...
		[[UIApplication sharedApplication] openURL:[NSURL URLWithString:NSLocalizedString(@"http://itunes.apple.com/us/app/edublogs/id526466328?mt=8", @"App URL, change 'us' for your local store code (test the link first)")]];
	}
}
