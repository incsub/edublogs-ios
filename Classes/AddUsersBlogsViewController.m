...
- (void)refreshBlogs {
    ...
    if (isWPcom) {
        NSError *error = nil;
        xmlrpc = [NSURL URLWithString:@"http://edublogs.org/xmlrpc.php"];
            ...
            success:^(AFHTTPRequestOperation *operation, id responseObject) {
                ...
                if(usersBlogs.count > 0) {
                    ...
                    [usersBlogs enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                        ...
                        NSString *strTitle = [NSString stringWithFormat:@"%@", title];
                        
                        NSLog(@"blogTitle: %@ %@", title, strTitle);
                        
                        if ( [strTitle isEqualToString:title] ) {
                            NSLog(@"Trying to decode Title");
                            title = [title stringByDecodingXMLCharacters];
                        } else {
                            title = @"[EMPTY]";
                        }
                        title = [title trim];
                        NSLog(@"blogTitle: %@", title);
