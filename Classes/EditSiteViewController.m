    ...
    if ([indexPath section] == 0) {
        if (indexPath.row == 0) {
            ...
            if (self.usernameCell == nil) {
				...
				usernameTextField.placeholder = NSLocalizedString(@"Edublogs Campus username", @"");
            ...
            if (self.passwordCell == nil) {
				...
				passwordTextField.placeholder = NSLocalizedString(@"Edublogs Campus password", @"");
    ...
    [WordPressApi guessXMLRPCURLForSite:urlToValidate success:^(NSURL *xmlrpcURL) {
        ...
        if ([error.domain isEqual:NSURLErrorDomain] && error.code == NSURLErrorUserCancelledAuthentication) {
            ...
            NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:
                                      NSLocalizedString(@"Couldn't find a Edublogs Campus site on that URL", @""),NSLocalizedDescriptionKey,
                                      nil];
            NSError *err = [NSError errorWithDomain:@"org.edublogs" code:NSURLErrorBadURL userInfo:userInfo];
