...
//#warning Need Flurry api key for distribution
	...
	[FileLogger log:@"Launching Edublogs for iOS %@...", [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"]];
    ...
    // allocate the Edublogs.org reachability object
    wpcomReachability = [Reachability reachabilityWithHostname:@"edublogs.org"];
    // set the blocks 
    wpcomReachability.reachableBlock = ^(Reachability*reach)
    {  
        WPLog(@"Edublogs REACHABLE!");
        self.wpcomAvailable = YES;
    };
    wpcomReachability.unreachableBlock = ^(Reachability*reach)
    {
        WPLog(@"Edublogs UNREACHABLE!");
...
- (void)showNotificationErrorAlert:(NSNotification *)notification {
	...
	if([[notification object] isKindOfClass:[NSError class]]) {
		...
		//org.edublogs --> XML-RPC errors
		if ([[err domain] isEqualToString:@"org.edublogs"]){
			if([err code] == 401)
...
- (void)showContentDetailViewController:(UIViewController *)viewController {
	if (self.splitViewController) {
        ...
        if (viewController) {
            ...
            fabricController.navigationItem.title = @"Edublogs";
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    NSURL *storeURL = [NSURL fileURLWithPath: [[self applicationDocumentsDirectory] stringByAppendingPathComponent: @"Edublogs.sqlite"]];
...
- (void)checkWPcomAuthentication {
	...
	NSString *authURL = @"http://edublogs.org/wp-login.php";
	...
	if([[NSUserDefaults standardUserDefaults] objectForKey:@"wpcom_username_preference"] != nil) {
        ...
        if (password != nil) {
            ...
            [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
                WPFLog(@"Authenticated in Edublogs.org, cached reader");
...
- (void)runStats {
	...
	NSMutableURLRequest *theRequest=[NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"http://edublogs.org/iphoneapp/update-check/1.0/"]
...
- (void) connectionDidFinishLoading: (NSURLConnection*) connection {
    ...
    NSLog(@"New version: %@", statsDataString);
    if ([statsDataString compare:appversion] > 0) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Update Available", @"")
                                                        message:NSLocalizedString(@"A new version of Edublogs for iOS is now available", @"")
                                                       delegate:self
                                              cancelButtonTitle:NSLocalizedString(@"Dismiss", @"")
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex { 
	...
	if (alertView.tag == 101) { //Password Alert
        ...
        if (buttonIndex == 1) {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://itunes.apple.com/us/app/edublogs/id526466328?mt=8&ls=1"]];
        }
