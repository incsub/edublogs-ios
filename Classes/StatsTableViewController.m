...
-(void)getUserAPIKey {
	...
	{
		[self showLoadingDialog];
		apiKeyConn = [[NSURLConnection alloc] initWithRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"https://public-api.edublogs.org/get-user-blogs/1.0"]] delegate:self];
	...
	else 
	{
		...
		UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Edublogs Stats", @"")
														 message:NSLocalizedString(@"To load stats for your blog you will need to have the Edublogs stats plugin installed and correctly configured as well as your Edublogs login.", @"") 
...
- (void)connection:(NSURLConnection *)connection didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
	...
	else if ([challenge previousFailureCount] <= 1)
	{
        ...
        if ([blog isWPcom]) {
            ...
            s_password = [SFHFKeychainUtils getPasswordForUsername:s_username andServiceName:@"Edublogs" error:&error];
            dotorgLogin = YES;
        }
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
	...
	if ([response respondsToSelector:@selector(statusCode)])
	{
		...
		if (statusCode >= 400)
		{
			...
			NSError *statusError = [NSError errorWithDomain:@"org.edublogs"
													   code:statusCode
												   userInfo:errorInfo];
