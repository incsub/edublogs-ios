...
+ (void)guessXMLRPCURLForSite:(NSString *)url
    ...
    if (url == nil || [url isEqualToString:@""]) {
        NSError *error = [NSError errorWithDomain:@"org.edublogs.api" code:0 userInfo:[NSDictionary dictionaryWithObject:NSLocalizedString(@"Empty URL", @"") forKey:NSLocalizedDescriptionKey]];
    ...
    if (xmlrpcURL == nil) {
        ...
        NSError *error = [NSError errorWithDomain:@"org.edublogs.api" code:1 userInfo:[NSDictionary dictionaryWithObject:NSLocalizedString(@"Invalid URL", @"") forKey:NSLocalizedDescriptionKey]];
        return failure ? failure(error) : nil;
    }
                            ...
                            if (!rsdError) {
                                @try {
                                    ...
                                    for(CXMLElement *api in [[[serviceXML elementsForName:@"apis"] objectAtIndex:0] elementsForName:@"api"]) {
                                        if([[[api attributeForName:@"name"] stringValue] isEqualToString:@"Edublogs"]) {
