...
+ (NSString *)titleForRemoteStatus:(NSNumber *)remoteStatus {
    switch ([remoteStatus intValue]) {
        ...
        case MediaRemoteStatusFailed:
            ...
            break;
        case MediaRemoteStatusTooBig:
            return NSLocalizedString(@"File exceeds the upload limit", @"");
            break;
...
- (void)fileTooBig {
    self.remoteStatus = MediaRemoteStatusTooBig;
    if (self.uploader) {
        [self.uploader stop];
    }
    [[self managedObjectContext] deleteObject:self];

    [self showAlertWithTitle:@"Error Uploading" message:@"File exceeds the upload limit"];
}
...
- (void)uploadWithSuccess:(void (^)())success failure:(void (^)(NSError *error))failure {
    [self save];
    self.progress = 0.0f;
    int MaxUploadLimit = 32*1000;
    int _fileSize = [self.filesize intValue];
    ...
    } else if (MaxUploadLimit >= _fileSize){
...
- (void)upload {
    [self uploadWithSuccess:^{
        if ([self isDeleted]) {
            ...
            NSLog(@"Media deleted while uploading (%@)", self);
            return;
        }
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ShouldInsertMediaBelow" object:self];
        [self save];
    } failure:nil];
}
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        if (operation.responseString != nil && ![operation.responseString isEmpty]) {
            if ([operation.responseString rangeOfString:@"AtomPub services are disabled"].location != NSNotFound) {
                if (failure) {
                    NSError *error = [NSError errorWithDomain:@"org.edublogs" code:0 userInfo:[NSDictionary dictionaryWithObject:operation.responseString forKey:NSLocalizedDescriptionKey]];
                    self.remoteStatus = MediaRemoteStatusFailed;
                    [_uploadOperation release]; _uploadOperation = nil;
                    failure(error);
                }
